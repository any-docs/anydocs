...
Using this property with a value other than `none` creates a new [stacking context](/css/guides/Positioned_layout/Stacking_context). Also, in that case, the object will act as a containing block for `position: fixed` elements that it contains.
...
An example showing how a cube varies if the `perspective` is set at different positions is given in [Using CSS transforms > Setting perspective](/css/guides/Transforms/Using#setting_perspective).
