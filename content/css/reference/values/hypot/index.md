...
The **`hypot()`** [CSS](/en-US/docs/Web/CSS) [function](/css/reference/values/Functions) is an exponential function that returns the [square root](https://en.wikipedia.org/wiki/Square_root) of the sum of squares of its parameters.

While {{CSSxRef("pow")}} and {{CSSxRef("sqrt")}} only work on unitless numbers, `hypot()` accepts values with units, but they all must have the same [type](/css/reference/values/Data_types).
