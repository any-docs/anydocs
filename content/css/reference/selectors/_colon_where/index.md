...
The difference between `:where()` and {{cssxref(":is()")}} is that `:where()` always has 0 [specificity](/css/guides/Cascade/Specificity), whereas `:is()` takes on the specificity of the most specific selector in its arguments.
...
The `:where()` pseudo-class requires a [selector list](/css/guides/Selectors/Selector_structure#selector_list), a comma-separated list of one or more selectors, as its argument. The list must not contain a [pseudo-element](/en-US/docs/Web/CSS/Reference/Selectors/Pseudo-elements), but any other simple, compound, and complex selectors are allowed.
