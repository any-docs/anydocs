...
In the above example we achieve the sticky footer using [CSS grid layout](/css/guides/Grid_layout). The `.wrapper` has a minimum height of `100%` which means it is as tall as the container it is in. We then create a single column grid layout with three rows, one row for each part of our layout.

[Grid auto-placement](/css/guides/Grid_layout/Auto-placement) will place our items in source order and so the header goes into the first auto sized track, the main content into the `1fr` track and the footer into the final auto sized track. The `1fr` track will take up all available space and so grows to fill the gap.

## Alternate method

You can also use [flexbox](/css/guides/Flexible_box_layout) to create a sticky footer.
...
The flexbox example starts out in the same way, but we use `display:flex` rather than `display:grid` on the `.wrapper`; we also set {{cssxref("flex-direction")}} to `column`. Then we set our main content to [`flex-grow: 1`](/css/reference/properties/flex-grow) and the other two elements to [`flex-shrink: 0`](/css/reference/properties/flex-shrink) — this prevents them from shrinking smaller when content fills the main area.
