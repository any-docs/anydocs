...
Anchor positioning changes the [containing block](/css/guides/Display/Containing_block) of anchor-positioned elements, making its `position` relative to its anchor rather than to the nearest positioned ancestor element.
...
You cannot associate a positioned element with an anchor element if the anchor is hidden, such as with {{cssxref("display", "display: none")}} or {{cssxref("visibility", "visibility: hidden")}}, or if the anchor is part of the [skipped contents](/css/guides/Containment/Using#skips_its_contents) of another element due to it having {{cssxref("content-visibility", "content-visibility: hidden")}} set on it.
...
For more information on anchor features and usage, see the [CSS anchor positioning](/css/guides/Anchor_positioning) module landing page and the [Using CSS anchor positioning](/css/guides/Anchor_positioning/Using) guide.
