...
In the case of the `animation-fill-mode` [forwards](/css/reference/properties/animation-fill-mode#forwards) value, animated properties behave as if included in a set {{cssxref("will-change")}} property value. If a new stacking context is created during the animation, the target element retains the stacking context after the animation has finished.
...
> [!NOTE]
> Animating [CSS Box Model](/css/guides/Box_model) properties is discouraged. Animating any box model property is inherently CPU intensive; consider animating the [transform](/css/reference/properties/transform) property instead.
...
See [Using CSS animations](/css/guides/Animations/Using#examples) for additional examples.
