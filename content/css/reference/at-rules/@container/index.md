...
The **`@container`** [CSS](/en-US/docs/Web/CSS) [at-rule](/css/guides/Syntax/At-rules) is a conditional group rule that applies styles to a [containment context](/css/guides/Containment/Container_queries#naming_containment_contexts).
...
The `<container-condition>` can include one or more boolean size queries, each within a set of parentheses. A size query includes a size descriptor, a value, and — depending on the descriptor — a comparison operator. The queries always measures the [content box](/css/reference/values/box-edge#content-box) as the comparison. The syntax for including multiple conditions is the same as for {{cssxref("@media")}} size feature queries.
...
- `snapped`
  - : Queries whether the container is going to be snapped to a [scroll snap](/css/guides/Scroll_snap) container ancestor along the given axis. Valid `snapped` values include the following keywords:
    - `none`
...
Container queries can also evaluate the computed style of the container element. A _container style query_ is a `@container` query that uses one or more `style()` functional notations. The boolean syntax and logic combining style features into a style query is the same as for [CSS feature queries](/css/guides/Conditional_rules/Using_feature_queries).
...
The parameter of each `style()` is a single `<style-feature>`. A **`<style-feature>`** is a valid CSS [declaration](/css/guides/Syntax/Introduction#css_declarations), a CSS property, or a [`<custom-property-name>`](/css/reference/values/var#values).
...
The following container query checks if the [computed value](/css/guides/Cascade/Property_value_processing#computed_value) of the container element's `--accent-color` is `blue`:
...
See [Using container scroll-state queries](/css/guides/Conditional_rules/Container_scroll-state_queries) for full walkthroughs of scroll-state query examples.
