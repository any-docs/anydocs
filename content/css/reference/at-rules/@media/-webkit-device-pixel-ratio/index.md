...
- {{cssxref("&lt;number&gt;")}}
  - : The number of device pixels used to represent each CSS [`px`](/css/reference/values/length#absolute_length_units). Although the value is a `<number>`, and thus doesn't syntactically allow units, its implicit unit is [`dppx`](/css/reference/values/resolution#units).
