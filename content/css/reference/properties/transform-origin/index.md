...
The transform origin is the point around which a transformation is applied. For example, the transform origin of the [`rotate()`](/css/reference/values/transform-function/rotate) function is the center of rotation.
...
The `transform-origin` property may be specified using one, two, or three values, where each value represents an offset. Offsets that are not explicitly defined are reset to their corresponding [initial values](/css/guides/Cascade/Property_value_processing#initial_value).
