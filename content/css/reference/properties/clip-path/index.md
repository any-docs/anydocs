...
- `<geometry-box>`
  - : If specified in combination with a `<basic-shape>`, this value defines the reference box for the basic shape. If specified by itself, it causes the edges of the specified box, including any corner shaping (such as a {{cssxref("border-radius")}}), to be the clipping path. The geometry box can be one of the following values:
    - `margin-box`
      - : Uses the [margin box](/css/guides/Shapes/From_box_values#margin-box) as the reference box.
    - `border-box`
      - : Uses the [border box](/css/guides/Shapes/From_box_values#border-box) as the reference box.
    - `padding-box`
      - : Uses the [padding box](/css/guides/Shapes/From_box_values#padding-box) as the reference box.
    - `content-box`
      - : Uses the [content box](/css/guides/Shapes/From_box_values#content-box) as the reference box.
    - `fill-box`
...
> [!NOTE]
> A computed value other than **`none`** results in the creation of a new [stacking context](/css/guides/Positioned_layout/Stacking_context) the same way that CSS {{cssxref("opacity")}} does for values other than `1`.
...
Because the `shape()` function allows using {{cssxref("percentage")}} values (and [custom properties](/css/reference/properties/--*) too), it is more robust.
...
We use [flexbox](/css/guides/Flexible_box_layout) to allow our elements to sit side-by-side with a gap between them, if there is space available. We define a {{cssxref("gradient/conic-gradient", "conic-gradient()")}} background image on both `<div>` elements, providing an interesting visual to clip, along with a {{cssxref("border")}}.
