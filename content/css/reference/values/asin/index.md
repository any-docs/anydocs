...
The **`asin()`** [CSS](/en-US/docs/Web/CSS) [function](/css/reference/values/Functions) is a trigonometric function that returns the inverse sine of a number between `-1` and `1`. The function contains a single calculation that returns an {{cssxref("angle")}} between `-90deg` and `90deg`.
