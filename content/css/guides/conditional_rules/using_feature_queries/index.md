...
CSS feature queries are part of the [CSS conditional rules](/css/guides/Conditional_rules) module, which also defines the media query {{cssxref("@media")}} at-rule. Feature queries behave similarly to [media queries](/css/guides/Media_queries/Using). The difference is that with a media query, you are testing something about the environment in which the web page is running, whereas with feature queries you are testing browser support for CSS features.
