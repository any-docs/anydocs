...
- `preserve-parent-color`
  - : In forced colors mode, if the {{cssxref("color")}} property inherits from its parent (i.e., there is no [cascaded value](/css/guides/Cascade/Introduction) or the cascaded value is `currentColor`, {{cssxref("inherit")}}, or another keyword that inherits from the parent), then it computes to the [used color](/css/guides/Cascade/Property_value_processing#used_value) of its parent's `color` property. In all other cases, it behaves the same as `none`.
