...
The specification also lists an additional value, `auto`, which is not currently supported in any browsers. When implemented, `auto` will compute to [`currentColor`](/css/reference/values/color_value#currentcolor_keyword) unless {{cssxref("outline-style")}} is set to `auto` then it will compute to the [accent color](/css/reference/properties/accent-color).
