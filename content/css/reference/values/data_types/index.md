...
The most commonly-used types are defined in the [CSS Values and Units](/css/guides/Values_and_units) module. This module also defines [functional notations](/css/reference/values/Functions), which allow for more complex types or processing. Other types are defined in the specifications to which they apply.
...
- {{cssxref("&lt;dashed-ident&gt;")}}
  - : A `<custom-ident>` with the additional restriction that it must start with two dashes, for example, with [CSS Custom Properties](/css/guides/Cascading_variables/Using_custom_properties).
- {{cssxref("&lt;string&gt;")}}
...
These data types are used to indicate quantities, indexes, and positions. The majority of these are defined in the CSS values and units module, however additional types are described in other modules where they are specific to that specification alone — for example the `fr` unit in the [CSS grid layout](/css/guides/Grid_layout) module.
...
- {{cssxref("&lt;flex&gt;")}}
  - : A flexible length introduced for [CSS grid layout](/css/guides/Grid_layout), written as a `<number>` with the `fr` unit attached and used for grid track sizing.
...
The [CSS color](/css/guides/Colors) module defines the {{cssxref("&lt;color&gt;")}} data type, and other types which relate to color in CSS.
...
The [CSS images](/css/guides/Images) module defines the data types which deal with images, including gradients.
...
These data types are used in [CSS math function](/css/reference/values/Functions#math_functions) calculations.
...
The [CSS shapes](/css/guides/Shapes) and [CSS borders and box decorations](/css/guides/Borders_and_box_decorations) modules define shape data types:

- {{cssxref("&lt;basic-shape>")}}
  - : Describes shape functions used in the {{cssxref("clip-path")}}, {{cssxref("shape-outside")}}, and {{cssxref("offset-path")}} properties.
- {{cssxref("&lt;corner-shape-value>")}}
  - : Describes the shape of a container corner. It is used by the {{cssxref("corner-shape")}} shorthand property and its [constituent properties](/css/reference/properties/corner-shape#constituent_properties) to specify the shape applied to affected container corners.
...
- [CSS functional notation](/css/reference/values/Functions)
