...
The **`@scope`** [CSS](/en-US/docs/Web/CSS) [at-rule](/css/guides/Syntax/At-rules) enables you to select elements in specific DOM subtrees, targeting elements precisely without writing overly-specific selectors that are hard to override, and without coupling your selectors too tightly to the DOM structure.
...
Because {{cssxref(":where()")}} has zero [specificity](/css/guides/Cascade/Specificity), bare selectors and `&` add zero weight. The specificity weight is determined by the rest of the selector.
...
`@scope` adds a new criterion to the [CSS cascade](/css/guides/Cascade): **scoping proximity**. This states that when two scopes have conflicting styles, the style that has the smallest number of hops up the DOM tree hierarchy to the scope root is applied. Let's look at an example to see what this means.
...
> [!NOTE]
> Scoping proximity overrules source order but is itself overridden by other, higher-priority criteria such as [importance](/css/reference/values/important), [layers](/en-US/docs/Learn_web_development/Core/Styling_basics/Cascade_layers), and [specificity](/css/guides/Cascade/Specificity).
