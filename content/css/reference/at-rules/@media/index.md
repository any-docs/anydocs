...
The **`@media`** [CSS](/en-US/docs/Web/CSS) [at-rule](/css/guides/Syntax/At-rules) can be used to apply part of a style sheet based on the result of one or more [media queries](/css/guides/Media_queries/Using). With it, you specify a media query and a block of CSS to apply to the document if and only if the media query matches the device on which the content is being used.
...
For a discussion of media query syntax, please see [Using media queries](/css/guides/Media_queries/Using#syntax).
...
- `print`
  - : Intended for paged material and documents viewed on a screen in print preview mode. (Please see [paged media](/css/guides/Paged_media) for information about formatting issues that are specific to these formats.)
- `screen`
...
To best accommodate people who adjust a site's text size, use [`em`](/css/guides/Values_and_units/Numeric_data_types)s when you need a {{cssxref("&lt;length&gt;")}} for your [media queries](/css/guides/Media_queries/Using).

Both [`em`](/css/guides/Values_and_units/Numeric_data_types) and [`px`](/css/guides/Values_and_units/Numeric_data_types) are valid units, but [`em`](/css/guides/Values_and_units/Numeric_data_types) works better if the user changes the browser text size.
...
For more examples, please see [Using media queries](/css/guides/Media_queries/Using).
