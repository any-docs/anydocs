...
The **`<color>`** [CSS](/en-US/docs/Web/CSS) [data type](/css/reference/values/Data_types) represents a color.
...
- By using [relative color](/css/guides/Colors/Using_relative_colors) syntax to output a new color based on an existing color. Any of the above color functions can take an **origin color** preceded by the `from` keyword and followed by definitions of the channel values for the new **output color**.
...
Color interpolation happens with [gradients](/css/reference/values/gradient), [transitions](/css/guides/Transitions/Using), and [animations](/css/guides/Animations/Using).

When interpolating `<color>` values, they are first converted to a given color space, and then each component of the [computed values](/css/guides/Cascade/Property_value_processing#computed_value) are interpolated linearly, with interpolation's speed being determined by the [easing function](/css/reference/values/easing-function) in transitions and animations. The interpolation color space defaults to Oklab, but can be overridden through {{CSSXref("&lt;color-interpolation-method&gt;")}} in some color-related functional notations.
