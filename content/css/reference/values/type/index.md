...
The **`type()`** [CSS](/en-US/docs/Web/CSS) [function](/css/reference/values/Functions) allows you to specify a particular data type, and is used by the following CSS features:

- The {{cssxref("attr()")}} function, to specify the data type that an attribute value should be parsed into.
- The {{cssxref("@function")}} at-rule, to specify the permitted data types for [CSS custom function](/css/guides/Custom_functions_and_mixins/Using_custom_functions) parameters and results.

> [!NOTE]
> The {{cssxref("@property")}} at-rule {{cssxref("@property/syntax", "syntax")}} descriptor value uses the same `<syntax>` to define the permitted data types for registered [custom properties](/css/reference/properties/--*), however, this always takes the form of a string.
